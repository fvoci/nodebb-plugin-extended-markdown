...
const codeTabRegex =
  /(?:<p dir="auto">={3}group<\/p>\n)((?:<pre><code class=".+">[^]*?<\/code><\/pre>\n){2,})(?:<p dir="auto">={3}<\/p>)/g
const langCodeRegex = /<code class="(.+)">/

const colorRegex =
  /(<code.*>*?[^]<\/code>)|%\((#[\dA-Fa-f]{6}|rgb\(\d{1,3}, ?\d{1,3}, ?\d{1,3}\)|[a-z]+)\)\[(.+?)]/g

const paragraphAndHeadingRegex = /<(h[1-6]|p dir="auto")>([^]*?)<\/(h[1-6]|p)>/g

const noteRegex =
  /<p dir="auto">!!! (info|warning|important) \[([a-zA-Z0-9]*)\]: ((.|<br \/>\n)*)<\/p>/g
...
const ExtendedMarkdown = {
  ...
  async parsePost(data) {
    if (data && data.postData && data.postData.content) {
      ...
      data.postData.content = applyGroupCode(
        data.postData.content,
        data.postData.pid
      )
      data.postData.content = applySpoiler(
        data.postData.content,
        data.postData.pid
      )
    }
    ...
    const formatting = [
      {
        ...
        title: '[[extendedmarkdown:composer.formatting.color]]'
      },
      {
        name: 'left',
...
function applyExtendedMarkdown(textContent) {
  if (textContent.match(noteRegex)) {
    textContent = textContent.replace(
      noteRegex,
      function (match, type, title, text) {
  ...
  if (textContent.match(tooltipRegex)) {
    textContent = textContent.replace(
      tooltipRegex,
      function (match, code, text, tooltipText) {
  ...
  if (textContent.match(colorRegex)) {
    textContent = textContent.replace(
      colorRegex,
      function (match, code, color, text) {
        if (typeof code !== 'undefined') {
          return code
        }
  ...
  if (textContent.match(paragraphAndHeadingRegex)) {
    textContent = textContent.replace(
      paragraphAndHeadingRegex,
      function (match, tag, text, closeTag) {
